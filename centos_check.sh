#!/usr/bin/env bash
# CentOS/Rocky Linux 24h Security Check (read-only)
# Output: /tmp/centos_24h_security_audit_<timestamp>.txt
...
REPORT="/tmp/centos_24h_security_audit_${TS}.txt"
...
# Prefer journalctl (waktu bisa difilter) + fallback secure log bila perlu
...
# KALKULASI RINGKAS (pakai journalctl; jika kosong, fallback grep secure log)
extract_or_fallback() {
  local pattern="$1"
  local out
  out=$(eval "$JCTL_SSH" | grep -F "$pattern" || true)
  if [[ -z "$out" ]]; then
    # fallback ke secure log (RHEL-based systems)
    out=$( ( $SUDO zgrep -h "$pattern" /var/log/secure* 2>/dev/null || true ) )
  fi
  printf "%s" "$out"
}
...
cmd "$SUDO systemctl status sshd --no-pager"
...
cmd "$SUDO bash -c 'find /etc/cron* /var/spool/cron -type f -newermt \"$SINCE_HUMAN\" -printf \"%TY-%Tm-%Td %TH:%TM  %u:%g  %p\n\" 2>/dev/null'"
cmd "systemctl list-timers --all"

# SELINUX STATUS (specific to RHEL-based)
hdr "SELinux status & denials (last 24h)"
cmd "$SUDO getenforce || echo 'SELinux not available'"
if command -v sealert >/dev/null 2>&1; then
  cmd "$SUDO sealert -a /var/log/audit/audit.log | tail -n 50 || true"
fi
cmd "$SUDO ausearch -m avc -ts $(date -d \"$SINCE_HUMAN\" +%m/%d/%Y) 2>/dev/null | tail -n 50 || echo 'No audit log or AVC denials found'"

# FIREWALL STATUS (firewalld for RHEL-based)
if command -v firewall-cmd >/dev/null 2>&1; then
  hdr "Firewalld status & recent changes"
  cmd "$SUDO firewall-cmd --state || true"
  cmd "$SUDO firewall-cmd --list-all || true"
  cmd "$SUDO bash -c '$JCTL -u firewalld | tail -n 50 || true'"
fi

# IPTABLES fallback
if command -v iptables >/dev/null 2>&1; then
  hdr "IPTables rules (current)"
  cmd "$SUDO iptables -L -n -v || true"
fi
...
# PACKAGE CHANGES (yum/dnf for RHEL-based)
if [[ -f /var/log/yum.log ]]; then
  hdr "YUM/DNF activity (last 24h)"
  cmd "$SUDO awk -v ts=\"$(date -d \"$SINCE_HUMAN\" +%b\\ %d)\" '\$1\" \"\$2>=ts {print}' /var/log/yum.log || true"
fi

if [[ -f /var/log/dnf.log ]]; then
  hdr "DNF activity (last 24h)"
  cmd "$SUDO awk -v ts=\"$(date -d \"$SINCE_HUMAN\" +%Y-%m-%d)\" '\$1>=ts {print}' /var/log/dnf.log || true"
fi

# RPM database changes
hdr "RPM database changes (packages installed/removed in last 24h)"
cmd "$SUDO find /var/lib/rpm -name '__db*' -newermt '$SINCE_HUMAN' -printf '%TY-%Tm-%Td %TH:%TM  %p\n' 2>/dev/null || echo 'None found'"

# USERS & GROUPS SNAPSHOT
hdr "Human users & wheel group members (snapshot)"
cmd "$SUDO awk -F: '\$3>=1000 && \$1!=\"nobody\" && \$1!=\"nfsnobody\"{print \$1\":\"\$7}' /etc/passwd"
cmd "$SUDO getent group wheel"
cmd "$SUDO getent group sudo 2>/dev/null || echo 'sudo group not found (using wheel)'"
...
# RHEL-SPECIFIC SECURITY CHECKS
hdr "Recent changes to security-related configs (last 24h)"
cmd "$SUDO bash -c 'find /etc/pam.d /etc/security -type f -newermt \"$SINCE_HUMAN\" -printf \"%TY-%Tm-%Td %TH:%TM  %u:%g  %p\n\" 2>/dev/null || echo \"None found\"'"

hdr "Recent systemd service changes (last 24h)"
cmd "$SUDO find /etc/systemd/system /usr/lib/systemd/system -name '*.service' -newermt '$SINCE_HUMAN' -printf '%TY-%Tm-%Td %TH:%TM  %u:%g  %p\n' 2>/dev/null | head -20 || echo 'None found'"

# AUDIT LOG ANALYSIS (if auditd is available)
if command -v aureport >/dev/null 2>&1; then
  hdr "Audit report summary (last 24h)"
  cmd "$SUDO aureport --start $(date -d \"$SINCE_HUMAN\" +%m/%d/%Y) --summary || true"
  
  hdr "Audit: Failed login attempts (last 24h)"
  cmd "$SUDO aureport --start $(date -d \"$SINCE_HUMAN\" +%m/%d/%Y) --login --failed || true"
  
  hdr "Audit: File access events (last 24h, top 20)"
  cmd "$SUDO aureport --start $(date -d \"$SINCE_HUMAN\" +%m/%d/%Y) --file | head -20 || true"
fi
